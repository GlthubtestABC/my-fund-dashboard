  ...
  return (
    <DashboardLayout>
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1}
        className="card-gradient rounded-xl border border-border p-4 mb-4 corner-accent relative">
        <div className="flex flex-wrap gap-2 items-end">
          <div className="flex-1 min-w-[120px]">
            ...
            <input value={newCode} onChange={(e) => setNewCode(e.target.value)} placeholder="如 110011"
              className="w-full bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring transition-shadow focus:shadow-[0_0_12px_hsl(var(--ring)/0.15)]"
          ...
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            onClick={handleAdd} className="flex items-center gap-1.5 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90 transition-opacity shadow-[0_0_12px_hsl(var(--primary)/0.2)]">
      ...
      {allTags.length > 0 && (
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={2} className="flex flex-wrap gap-2 mb-4">
          <Tag className="w-4 h-4 text-muted-foreground mt-1" />
          {allTags.map((tag) => (
            <motion.button key={tag} onClick={() => toggleTag(tag)} whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.95 }}
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${selectedTags.includes(tag) ? "bg-primary text-primary-foreground shadow-[0_0_12px_hsl(var(--primary)/0.3)]" : "bg-secondary text-secondary-foreground hover:bg-accent"}`}>
              {tag}
            </motion.button>
          ))}
        </motion.div>
      ...
      <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid gap-4">
        <AnimatePresence>
          {filtered.map((fund, idx) => {
            const sparkColor = (fund.change1m ?? 0) >= 0 ? "hsl(0, 85%, 55%)" : "hsl(140, 60%, 38%)";
            const riskStyle = riskColors[fund.riskLevel] || "text-muted-foreground bg-muted";
            return (
              <motion.div key={fund.code} variants={cardVariant} custom={idx} layout
                className="card-gradient rounded-xl border border-border hover:border-primary/30 transition-all group overflow-hidden relative corner-accent border-glow"
              ...
              >
                ...
                <div className="grid grid-cols-7 gap-0 mx-5 mt-2 rounded-lg bg-muted/50 overflow-hidden">
                  {[
                    ...
                    <div key={i} className="text-center py-2.5 border-r border-border/30 last:border-r-0">
                ...
                <div className="flex items-center justify-between px-5 py-3 mt-2 border-t border-border/30">
                  ...
                  {(fund.alertNavBelow || fund.alertGainAbove) && (
                    <div className="flex items-center gap-1.5">
                      <Bell className="w-3 h-3 text-warning data-tick" />
