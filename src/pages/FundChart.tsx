...
import {
  PRESET_BENCHMARKS, createCustomBenchmark, generateBenchmarkCurve,
  loadBenchmarkConfig, saveBenchmarkConfig, BenchmarkConfig, Benchmark,
...
const FundChart = () => {
  ...
  const data = useMemo(() => {
    ...
    return history.map((d, i) => {
      ...
      const row: Record<string, any> = { date: d.date, fundReturn };
      for (const bench of activeBenchmarks) {
    ...
    <DashboardLayout>
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1} className="flex flex-wrap gap-3 mb-4">
        ...
        <div className="flex bg-secondary rounded-lg p-0.5">
          {periods.map((p, i) => (
            <motion.button key={p.label} onClick={() => setPeriodIdx(i)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${periodIdx === i ? "bg-primary text-primary-foreground shadow-[0_0_12px_hsl(var(--primary)/0.3)]" : "text-secondary-foreground hover:text-foreground"}`}>
              {p.label}
            </motion.button>
          ))}
        </div>
        ...
        <div className="flex bg-secondary rounded-lg p-0.5">
          <motion.button onClick={() => setMode("cumulative")} whileTap={{ scale: 0.95 }}
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${mode === "cumulative" ? "bg-primary text-primary-foreground" : "text-secondary-foreground"}`}>
            累计涨幅
          </motion.button>
          <motion.button onClick={() => setMode("annualized")} whileTap={{ scale: 0.95 }}
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${mode === "annualized" ? "bg-primary text-primary-foreground" : "text-secondary-foreground"}`}>
            年化涨幅
          </motion.button>
        </div>
      </motion.div>
      ...
      {activeBenchmarks.length > 0 && (
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={2} className="flex flex-wrap gap-1.5 mb-4">
